  ...
  if (!groups.length) {
    return (
      <SectionPanel className="flex items-center gap-4">
        ...
        <div>
          <h3 className="text-base font-semibold text-white">No change events yet</h3>
          <p className="text-sm text-slate-300/80">Canon diffs will appear here the first time a record moves.</p>
        </div>
      </SectionPanel>
    );
  }
            ...
            {group.entries.map((entry) => (
              <div key={entry.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <p className="text-xs uppercase tracking-[0.22em] text-slate-400/80">
                    v{entry.entity_version_before} to v{entry.entity_version_after}
                  </p>
                </div>
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border border-rose-300/12 bg-rose-300/[0.05] p-3">
                    <p className="mb-2 text-[11px] uppercase tracking-[0.24em] text-rose-100/80">Before</p>
                    <pre
                      className={cn(
                        "overflow-x-auto whitespace-pre-wrap break-words text-sm leading-6 text-slate-200/88",
                  ...
                  <div className="rounded-2xl border border-cyan-300/12 bg-cyan-300/[0.05] p-3">
                    <p className="mb-2 text-[11px] uppercase tracking-[0.24em] text-cyan-100/80">After</p>
                    <pre
                      className={cn(
